...
func _calc_grabber_loc():
	...
	to_return.x = get_rect().size.x * percent 
	to_return.y = get_rect().size.y / 2

	return to_return

func draw_grabber(pos):
	draw_circle(pos, self.get_rect().size.y/2, _border_color)
	draw_circle(pos, (self.get_rect().size.y/2) * .90, _grabber_color)
